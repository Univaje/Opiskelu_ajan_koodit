...
; CONFIG3H
  ...
  CONFIG  MSSPMSK = MSK7        ; MSSP Address Masking Mode Select bit (7-Bit Address Masking mode enable)
   ...
   ;subrutine will be executed when call function is made
sub1:
    ;movlw moves bits aa to working register bits aa is 10101010
    movlw 0xaa
    ;movfw moves bits from working register to portA
    movwf PORTA
    return
...
start:
    ;clears trisb as output if it is se to ones its input
    movlw 0xf0
    movwf TRISB
    ...
    setf TRISB
    ;second way to set trisb as output
    clrf TRISB
