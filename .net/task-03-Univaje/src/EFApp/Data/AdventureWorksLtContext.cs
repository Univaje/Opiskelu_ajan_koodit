...
//2. Add a method named `ProductCategoryItemsListPriceSumAsync` to DbContext class to count the sum of list prices on all products in selected category. 
// The method takes the product category id as input parameter. The method is asynchronous.

    public async Task<decimal>ProductCategoryItemsListPriceSumAsync(int productCategoryId)
    {
        decimal sum = await Products.Where(p => p.ProductCategoryId == productCategoryId).SumAsync(p => p.ListPrice);
        return sum;
        
    }

//3. Add a method named `CustomerAddressesAsync` to DbContext class to return the address entities of a customer. 
// The method takes company name as parameter (field CompanyName). The parameter casing is not relevant and the method must work with any casing. 
//The method is asynchronous.

    public async Task<List<Address>> CustomerAddressesAsync(string companyName)
    {
        var addresses = await Addresses.Where(a => a.CustomerAddresses.Any(ca => ca.Customer.CompanyName.ToLower() == companyName.ToLower())).ToListAsync();
        return addresses;
    }


//4. Add a method named `ProductCategoryListAsync` to DbContext class to return a list of strings (`List<string>`) 
//that contains product categories with their sub-categories like:
  ...
  //  The main categories are surrounded with ++ ++ characters and the sub-categories are listed right after the main category ordered ascending by category id. 
  //The main category id is appended to sub-category id (i.e. `mainId.subId:`). The method is asynchronous.

    public async Task<List<string>> ProductCategoryListAsync()
    {
        var categories = await ProductCategories.OrderBy(pc => pc.ProductCategoryId).ToListAsync();
        var categoryList = new List<string>();
        foreach (var category in categories)
        {
            if (category.ParentProductCategoryId == null)
            {
                categoryList.Add($"++{category.ProductCategoryId}: {category.Name}++");
                foreach (var subCategory in categories)
                {
                    if (subCategory.ParentProductCategoryId == category.ProductCategoryId)
                    {
                        categoryList.Add($"{category.ProductCategoryId}.{subCategory.ProductCategoryId}: {subCategory.Name}");
                    }
                }
            }
        }
    ...
    //5. Add a method named `GetCustomerInfoAsync` to DbContext class to return a nullable `CustomerInfo` object with all available properties populated 
//(CustomerInfo class is in EFApp/Models folder). The method takes the customer's id and an address type as parameters. 
//If customer is not found by the id then `null` is returned. If the address type is not found then the address properties are not populated. 
//The method must not fail in any case. The method is asynchronous.

    public async Task<CustomerInfo?> GetCustomerInfoAsync(int customerId, string addressType)
    {
     try 
     {
        ...
        customerInfo.AddressLine1 = customer.CustomerAddresses.FirstOrDefault(ca => ca.AddressType == addressType)?.Address.AddressLine1 ?? null;
        customerInfo.City = customer.CustomerAddresses.FirstOrDefault(ca => ca.AddressType == addressType)?.Address.City ?? null;
        return customerInfo;
     }
     catch (Exception)
     {
...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
        => optionsBuilder.UseSqlServer("Server=tcp:sav-mik-devsql.database.windows.net,1433;Initial Catalog=AdventureWorksLT;Persist Security Info=False;User ID=ets730021s;Password=qMA9m7b4ZGfEgWsM;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30");
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.HasKey(e => e.SystemInformationId).HasName("PK__BuildVer__35E58ECA48EC4566");
