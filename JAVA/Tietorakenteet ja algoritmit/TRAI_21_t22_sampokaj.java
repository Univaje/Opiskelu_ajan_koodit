...
public class TRAI_21_t22 {
    ...
    public static <E> HashMap<E, Integer> esiintymat(Collection<E> C) {
        ...
        HashMap<E, Integer> esiintymat = new HashMap<>();//O(1)
        for (E raataja :C) {                             //O(n)
            if (esiintymat.containsKey(raataja)){        //O(1)
                Integer arvo = esiintymat.get(raataja);  //O(1)
                esiintymat.put(raataja,arvo+1);          //O(1)
            }
            else
                esiintymat.put(raataja,1);               //O(1)
        }
        return esiintymat;                               //O(1)
    }
